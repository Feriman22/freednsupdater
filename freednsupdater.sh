...
All logs are stored in /tmp/externalip.txt file inside the container.

Exit codes:
70: APIURL variable is missing
71: You have multiple DDNS addresses, and not defined the DDNSDOMAIN variable
72: curl command not found (not installed or not placed in default paths)
73: Do not flood anybody. You set the sleep time too low. It's a built in feature to avoid overload afraid.org or any IP check provider.
"

# Variables
[ ! "$APIURL" ] && echo "$(date) - ERROR: APIURL variable is missing. Exit." | tee "$LOG" && exit 70
[[ "$REPEATSEC" != ?(-)+([0-9]) ]] && REPEATSEC=300  # Set default if not exists or invalid
IPSTORE="/tmp/externalip.txt"
LOG="/tmp/afraid-ddns-ip-updater.log"

# Generate variables
[ "$(curl -s "$APIURL" | wc -l)" -gt "1" ] && [ ! "$DDNSDOMAIN" ] && echo "$(date) - ERROR: You have multiple DDNS addresses, and not defined the DDNSDOMAIN variable. Exit." | tee "$LOG" && exit 71
...
# Avoid flood
[ "$REPEATSEC" -lt "10" ] && echo "$(date) - ERROR: Do not flood anybody. Exit." | tee "$LOG" && exit 73
...
while true; do
	...
	if [ "$IPUPDATED" ]; then
		...
		if [ "$CURRENT_IP" == "$DDNSIP" ]; then
			...
			echo "$(date) - WARNING: IP update failed this time (too low REPEATSEC value?). Maybe next time." | tee "$LOG"
			unset DDNSIP
		fi
	fi
