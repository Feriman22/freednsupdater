...
# Main variables - LOG and IPSTORE defined first so they are available for all error messages
LOG="/tmp/afraid-ddns-ip-updater.log"
IPSTORE="/tmp/externalip.txt"

[ ! "$APIURL" ] && echo "$(date) - ERROR: APIURL variable is missing. Exit." | tee -a "$LOG" && exit 70
[[ "$CheckAgainInXSec" != ?(-)+([0-9]) ]] && CheckAgainInXSec=300  # Set default if not exists or invalid

# wget command verification - must come before any wget usage
! command -v wget > /dev/null && echo "$(date) - ERROR: wget command not found. Exit." | tee -a "$LOG" && exit 73

# Avoid flooding
[ "$CheckAgainInXSec" -lt "10" ] && echo "$(date) - ERROR: Do not flood anyone. You have set the CheckAgainInXSec value too low. Exit." | tee -a "$LOG" && exit 74

# Validate & generate variables - fetch APIURL only once
APIDATA="$(wget -qO- "$APIURL")"
[ "$(echo "$APIDATA" | grep -c 'afraid.org')" -gt "1" ] && [ ! "$DDNSDOMAIN" ] && echo "$(date) - ERROR: You have multiple DDNS addresses, and have not defined the DDNSDOMAIN variable. Exit." | tee -a "$LOG" && exit 71
[ ! "$DDNSDOMAIN" ] && DDNSDOMAIN="$(echo "$APIDATA" | cut -d '|' -f 1)"
[ ! "$DDNSDOMAIN" ] && echo "$(date) - ERROR: Failed to get DDNSDOMAIN variable. Do you have an Internet connection? Exit." | tee -a "$LOG" && exit 72
UPDATEURL="$(echo "$APIDATA" | grep "$DDNSDOMAIN" | cut -d '|' -f 3)"
unset APIDATA
...
updateip() {
	echo "$(date) - INFO: Updating IP for $DDNSDOMAIN..." | tee -a "$LOG"
	wget -qO- "$UPDATEURL"
	IPUPDATED="true"
}
...
getddnsip() {
	...
	! validateip "$DDNSIP" && echo "$(date) - WARNING: Invalid DDNS IP ($DDNSIP) for $DDNSDOMAIN." | tee -a "$LOG"
}

# Core of the script - run periodically
while true; do
	# List of IP providers (HTTPS to prevent MITM attacks on IP detection)
	providers=("https://ident.me" "https://ipinfo.io/ip" "https://ifconfig.me" "https://api.ipify.org")
	# Shuffle the array
	mapfile -t shuffled_providers < <(shuf -e "${providers[@]}")
	...
	for provider in "${shuffled_providers[@]}"; do
		...
		if validateip "$CURRENT_IP"; then
			...
			echo "$(date) - WARNING: $provider is not working now, the IP is invalid ($CURRENT_IP). Try to the next one." | tee -a "$LOG" && continue
		fi
	done

	# Validate that we have a valid IP from one IP provider
	if validateip "$CURRENT_IP"; then
		# Enter here only first time
		if [ ! -f "$IPSTORE" ]; then
			echo "$(date) - INFO: Script has been started. Check the IP of $DDNSDOMAIN" | tee -a "$LOG"
			getddnsip
			if [ "$CURRENT_IP" == "$DDNSIP" ]; then
				echo "$(date) - INFO: The IP already set for $DDNSDOMAIN." | tee -a "$LOG"
		...
		# Check if the stored IP is different than the current IP
			...
			if wget -q -O /dev/null https://freedns.afraid.org > /dev/null; then
				...
				echo "$(date) - ERROR: freedns.afraid.org is not available at this time." | tee -a "$LOG"
			fi
		fi
	else
		echo "$(date) - WARNING: We don't have a valid IP from any provider. Current value of IP is $CURRENT_IP" | tee -a "$LOG"
	fi
	...
	if [ "$IPUPDATED" ]; then
		getddnsip
		if [ "$CURRENT_IP" == "$DDNSIP" ]; then
			echo "$(date) - INFO: Verification completed. The $CURRENT_IP IP has been set for $DDNSDOMAIN" | tee -a "$LOG"
			echo "$CURRENT_IP" > "$IPSTORE"
			unset IPUPDATED DDNSIP
		else
			echo "$(date) - WARNING: IP update failed this time (too low CheckAgainInXSec value?). Maybe next time." | tee -a "$LOG"
			unset DDNSIP
		fi
	fi
